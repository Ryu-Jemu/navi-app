import React from 'react';
import './BoardPage.css';
import BottomNav from '../BottomNav/BottomNav';
import SearchIcon from '../assets/Search_Icon.png';
import ProfileIcon from '../assets/Profile_Icon.png';
import PostIcon from '../assets/List_Icon.png';

const BoardPage = () => {
  return (
    <div className="board-page">
      <header className="board-header">
        <span className="logo-text">Navi</span>
        <span className="board-title">Board</span>
        <div className="header-icons">
          <img src={SearchIcon} alt="Search" />
          <img src={ProfileIcon} alt="Profile" />
        </div>
      </header>

      <div className="language-tabs">
        <span className="tab active">English</span>
        <span className="tab">Chinese</span>
      </div>

      <div className="board-menu">
        <div className="menu-item">
          <img src={PostIcon} alt="My Posts" />
          <span>My Posts</span>
        </div>
        <div className="menu-item">
          <img src={PostIcon} alt="Commented" />
          <span>Commented</span>
        </div>
        <div className="menu-item">
          <img src={PostIcon} alt="Scrap" />
          <span>Scrap</span>
        </div>
        <hr className="divider" />
        <div className="menu-item">
          <img src={PostIcon} alt="Free Board" />
          <span>Free Board</span>
        </div>
      </div>

      <button className="add-post-button">Add Post</button>
      <BottomNav />
    </div>
  );
};

export default BoardPage;